...
!1B/D !2B 3C 4D 5D 6C 7C 8D 9C 10D 11B 12B !13C 14A 15B 16C !17A !18B 19C !20C 21D 22C 23C 24B !25D 26B 27D 28B

20
1条机器指令=4+取指微指令
μPC 8位 2^8=256微指令
4*64+取指微指令>256条 矛盾,所以选C

控制器容量
多少条微指令 μPC 2^8=256*32=2^8*2^5=2^13位
2^13/2^3=2^10B=1KB
...
!1D 2A !3A/B !4B/C !5D 6D 7A !8D/B !9C/D 10B !11A 12D 13D !14C/D 15C 16B 17D 18D 19B 20A 21A 22D 23C 24D 25B 26C 27A 28D 29C 30B 31C !32D/C
...
时空图画法:纵坐标指令,从上到下指令顺序执行;横坐标时钟周期,填充指令的流水段

I1      IF      ID      EX      M       WB
I2              IF      ID      EX      M       WB
I3                      IF      ID      EX      M       WB
不使用数据旁路的话,I1和I3数据冒险,因为I3的EX阶段需要r1,而r1在WB结束后才存入寄存器
使用数据旁路,I1的EX的结果r1,本来在M-WB之间的锁存器,直接转发到I3的EX阶段的ALU的输入即可,这样无需填充空操作

本来I3的EX阶段要使用r3,但是I2产生r3在M阶段,这两个是同一个阶段,所以这里要使用一个空操作
...
此时产生一个疑问,添加了空操作以后,I3这样看来好像不需要使用数据旁路,但是实际上还是需要数据旁路的,因为如果不使用数据旁路,那么I3应该在ID阶段就取出r1,但是I1此时处于WB阶段,同一个阶段,无法取到r1,所以结论是还是需要数据旁路
...
23
吞吐率=n/(k+n-1)dt=100/(4+100-1)dt
dt=1时钟周期=1/主频
主频=1.03*10^9
吞吐率=(100/103)*(1.03)*10^9=1*10^9 条/秒

31
时空图
I1      IF      ID      EX      M       WB
I1的EX专用通路到I2的EX,数据旁路,无需空操作
I2              IF      ID      EX      M       WB
I2的M产生的s3需要存入I3的EX的ALU,所以这里添加空操作
空                      nop     nop     nop     nop     nop
I3                              IF      ID      EX      M       WB
I3条件跳转,所以需要插入空操作三次
空                                      nop     nop     nop     nop     nop
...
1B 2A 3C 4C !5B/C 6A 7D 8C 9C 10A !11D/A 12B !13D 14C 15B 16B 17B 18B 19C 20B 22C 25C !26A/B 27C !28B

12
32位=4B
传递4B需要5个时钟周期,也就是5*(1/500M)秒
也就是1/100M秒
传递1B需要1/100M除以4
等于1/400M秒
那么一秒传递多少B?
...
65536=2^16
640*480*16=4915200位
这里的56kb/s,指的是每秒传输56*10^3位
4915200/56*10^3=87.77秒
...
20
总线周期=2*时钟周期
工作频率=时钟频率/2=10M/2=5M赫兹
总线带宽=5M*4B=20MB/s

25
总线周期=时钟周期/2
工作频率=时钟频率*2=132M赫兹
总线带宽=132M*4B=528MB/s

26
带宽=3通道*1333M*8B=31.2GB/s

27
总线周期=时钟周期/2
工作频率=时钟频率*2=2.4G*2=4.8G赫兹
带宽=4.8G*2B*2=19.2GB/s

28
总线周期=时钟周期/2
工作频率=时钟频率*2=420M*2=840M赫兹
带宽=840M*8B=6720MB/s=6.72GB/s

## 6.2.3

1C !2C/D 3C 4C 5D 6B !7D 8C 9C 10A !11C/B !12B/C 13C 14C 15A !16C/D !17D/B !18C 19D
...
128/32=4次
4时钟周期
一次 1/100M 秒=0.01*10^-6=10纳秒
传输数据 4/100M=0.04*10^-6=40*10^-9=40纳秒
传输地址需要 10纳秒,因为突发传输,一个时钟周期即可
一共10+40=50纳秒
...
!1C/A 2D 3D 4C 5B 6D 7D 8C 9D !10B !11A !12D 13B 14A 15D 16D !17A/D !18C/A

## 7.3.4

1B !2C !3D !4C !5A/B 6B 7C 8C 9B !10B/A 11B 12C 13A !14D !15C/A !16D/C !17A/B 18C !19B/D 20B !21C/BD !22C 23C !24C 25A 26A 27B !28A !29C !30C !31D/A 32B 33B !34C/D !35B/D 36C 37A 38A 39D !40C 41B !42D !43B !44B !45B !46D/C !47 !48C/D !49B/C 50B 51C !52A 53C !54C/D 55C !56B/A !57D/C
...
40
每秒时钟周期数量=200*500=10 00 00
占比=100000/50M=1*10^5/50*10^6=0.02*10^-1=0.002=0.2%
